...
public class Trie<V>: Codable where V: Hashable, V: Codable {
    private var children = OrderedDictionary<String, Trie>()
    ...
    enum CodingKeys: CodingKey {
        case children, exactMatchValues, substringMatchValues
    }
    
    public func encode(to encoder: any Encoder) throws {
        var container: KeyedEncodingContainer<Trie<V>.CodingKeys> = encoder.container(keyedBy: Trie<V>.CodingKeys.self)
        if !children.isEmpty {
            try container.encode(self.children, forKey: Trie<V>.CodingKeys.children)
        }
        if !exactMatchValues.isEmpty {
            try container.encode(self.exactMatchValues, forKey: Trie<V>.CodingKeys.exactMatchValues)
        }
        if !substringMatchValues.isEmpty {
            try container.encode(self.substringMatchValues, forKey: Trie<V>.CodingKeys.substringMatchValues)
        }
    }
    
    public required init(from decoder: any Decoder) throws {
        let container = try decoder.container(keyedBy: Trie<V>.CodingKeys.self)
        children = try container.decodeIfPresent(OrderedDictionary<String, Trie>.self, forKey: .children) ?? [:]
        exactMatchValues = try container.decodeIfPresent(OrderedSet<V>.self, forKey: .exactMatchValues) ?? []
        substringMatchValues = try container.decodeIfPresent(OrderedSet<V>.self, forKey: .substringMatchValues) ?? []
...
public struct TrieInsertionOptions: OptionSet, Sendable {
...
public extension Trie {
    ...
    func find(key: String, limit: Int? = nil) -> [V] {
        var currentNode = self

        // Find branch with matching prefix.
        for char in key {
            if let child = currentNode.children[String(char)] {
                currentNode = child
            } else {
                return []
            }
        ...
        while !queue.isEmpty,
              (currentNode.exactMatchValues.count + substringMatches.count) < (limit ?? Int.max) {
            ...
            for keyIndex in 0..<key.count {
                ...
                for char in key[key.index(key.startIndex, offsetBy: keyIndex)...] {
                    if let child = currentNode.children[String(char)] {
                        ...
                        currentNode.children[String(char)] = child
                        currentNode = child
                    }
                }
                ...
                if keyIndex == 0 {
                    ...
                    // If includeNonPrefixedMatches is not true, the first character of the key can be the only root branch
            ...
            if foundLeafNode {
                ...
                for keySubIndex in (keyIndex..<key.count).reversed() {
                    if let parent = currentNode.parent, !currentNode.hasValues && !currentNode.hasChildren {
                        currentNode = parent
                        let char = key[key.index(key.startIndex, offsetBy: keySubIndex)]
                        currentNode.children.removeValue(forKey: String(char))
                    }
                }
            }
        }
